        ...
        PaisaScreen {
            if let dashboard {
                ...
                PaisaGlassGroup(spacing: 14) {
                    PaisaMetricTile(
                        title: "Balance",
                        amount: BillBanditFormatters.currency(dashboard.totalOwed - dashboard.totalOwe, code: dashboard.currency),
                        tone: PaisaTheme.primaryBlue
                    )
                }
                ...
                PaisaPrimaryButton(
                    ...
                    isDisabled: dashboard.groups.isEmpty,
                    action: { createExpense(from: dashboard.groups) }
                )
                .accessibilityIdentifier("dashboard.continue")
...
private struct DashboardHero: View {
    ...
    var body: some View {
        PaisaPanel(tint: PaisaTheme.primaryBlue.opacity(0.18)) {
            VStack(alignment: .leading, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("Net balance")
                        ...
                        .foregroundStyle(.secondary)
                    Text(BillBanditFormatters.currency(netBalance, code: currency))
                        .font(.system(size: 30, weight: .bold, design: .rounded))
                        .foregroundStyle(PaisaTheme.primaryBlue)
                        .lineLimit(1)
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .trailing, spacing: 8) {
                Text(BillBanditFormatters.currency(abs(balance.amount), code: currency))
                    .font(.headline.weight(.semibold))
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(alignment: .firstTextBaseline) {
                    Text(expense.description)
                        ...
                        .foregroundStyle(PaisaTheme.ink)
                    Spacer()
                    Text(BillBanditFormatters.currency(expense.amount, code: expense.currency))
                ...
                HStack(spacing: 8) {
                    ...
                    Text("•")
                    Text(BillBanditFormatters.day(expense.date))
                }
                .font(.caption)
                .foregroundStyle(.secondary)
            }
