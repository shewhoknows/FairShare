                ...
                VStack(spacing: 12) {
                    ForEach(balances?.netBalances ?? []) { balance in
                        HStack {
                            ...
                            Spacer()
                            Text(BillBanditFormatters.currency(balance.netAmount, code: group.currency))
                                .font(.subheadline.weight(.semibold))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 10) {
                ...
                PaisaSecondaryButton("Remove", systemImage: "trash", tint: PaisaTheme.coral, action: onDelete)
                    .disabled(!canRemove)
                    .opacity(canRemove ? 1 : 0.55)
                    .accessibilityLabel("Remove expense \(expense.description)")
                    .accessibilityHint(canRemove ? "Deletes this expense from the group." : "Expense removal is currently unavailable.")
            }
        }
    }
}
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(alignment: .trailing, spacing: 8) {
                Text(BillBanditFormatters.currency(debt.amount, code: currency))
                    .font(.headline.weight(.semibold))
