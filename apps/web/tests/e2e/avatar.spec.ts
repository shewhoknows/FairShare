...
import { AUTH_A, waitForAppReady } from './helpers'
...
test.describe.serial('Avatar Picker', () => {
  ...
  test('2.2 modal shows 9 avatar options', async ({ page }) => {
    ...
    const avatarButtons = page.getByRole('dialog').getByRole('button', { name: /select .* avatar/i })
  ...
  test('2.4 selecting avatar updates preview', async ({ page }) => {
    ...
    const firstOption = page.getByRole('dialog').getByRole('button', { name: /select .* avatar/i }).first()
  ...
  test('2.5 save avatar updates sidebar and persists on reload', async ({ page }) => {
    ...
    const options = page.getByRole('dialog').getByRole('button', { name: /select .* avatar/i })
