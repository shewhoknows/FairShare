...
test.describe.serial('Comments', () => {
  test('8.0 setup — create group and add expense', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Expense', exact: true })).toBeVisible({ timeout: 10_000 })
  ...
  test('8.1 expand expense shows comment section', async ({ page }) => {
    ...
    const card = page.getByTestId('expense-card').filter({ hasText: expenseDesc }).first()
